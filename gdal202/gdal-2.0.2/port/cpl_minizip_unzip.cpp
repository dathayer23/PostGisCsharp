                                      ...
                                      voidpf filestream)
{
    ...
    if (ZSEEK(*pzlib_filefunc_def,filestream, uPosFound,ZLIB_FILEFUNC_SEEK_SET)!=0)
        return 0;
    ...
    if (unzlocal_getLong64(pzlib_filefunc_def,filestream,&relativeOffset)!=UNZ_OK)
        return 0;
    ...
    if (ZSEEK(*pzlib_filefunc_def,filestream, relativeOffset,ZLIB_FILEFUNC_SEEK_SET)!=0)
        return 0;
                                     ...
                                     zlib_filefunc_def* pzlib_filefunc_def)
{
    ...
    if (central_pos)
    {
        ...
        /* number of this disk */
        if (unzlocal_getLong(&us.z_filefunc, us.filestream,&number_disk)!=UNZ_OK)
            err=UNZ_ERRNO;

        /* number of the disk with the start of the central directory */
        if (unzlocal_getLong(&us.z_filefunc, us.filestream,&number_disk_with_CD)!=UNZ_OK)
            err=UNZ_ERRNO;

        /* total number of entries in the central directory on this disk */
        if (unzlocal_getLong64(&us.z_filefunc, us.filestream,&us.gi.number_entry)!=UNZ_OK)
            err=UNZ_ERRNO;

        /* total number of entries in the central directory */
        if (unzlocal_getLong64(&us.z_filefunc, us.filestream,&number_entry_CD)!=UNZ_OK)
            err=UNZ_ERRNO;
        ...
        if (unzlocal_getLong64(&us.z_filefunc, us.filestream,&us.size_central_dir)!=UNZ_OK)
            err=UNZ_ERRNO;

        /* offset of start of central directory with respect to the
          starting disk number */
        if (unzlocal_getLong64(&us.z_filefunc, us.filestream,&us.offset_central_dir)!=UNZ_OK)
            err=UNZ_ERRNO;
    ...
    *s=us;
    cpl_unzGoToFirstFile((unzFile)s);
    return (unzFile)s;
}
...
/*
  ...
  return UNZ_OK if there is no problem. */
extern int ZEXPORT cpl_unzGetGlobalInfo (unzFile file, unz_global_info* pglobal_info)
{
    unz_s* s;
    if (file==NULL)
        return UNZ_PARAMERROR;
                                               ...
                                               CPL_UNUSED uLong commentBufferSize)
{
    ...
    if ((err==UNZ_OK) && (file_info.size_file_extra != 0))
    {
        ...
        {
            uLong headerId;
            if (unzlocal_getShort(&s->z_filefunc, s->filestream,&headerId) != UNZ_OK)
                err=UNZ_ERRNO;

            uLong dataSize;
            if (unzlocal_getShort(&s->z_filefunc, s->filestream,&dataSize) != UNZ_OK)
                err=UNZ_ERRNO;
            ...
            {
                if (ZSEEK(s->z_filefunc, s->filestream,dataSize,ZLIB_FILEFUNC_SEEK_CUR)!=0)
                    err=UNZ_ERRNO;
            }
...
extern int ZEXPORT cpl_unzLocateFile (unzFile file, const char *szFileName, int iCaseSensitivity)
{
    ...
    while (err == UNZ_OK)
    {
        ...
        err = cpl_unzGetCurrentFileInfo(file,NULL,
                                    szCurrentFileName,sizeof(szCurrentFileName)-1,
                                    NULL,0,NULL,0);
        if (err == UNZ_OK)
        {
            if (cpl_unzStringFileNameCompare(szCurrentFileName,
...
{
    return cpl_unzOpenCurrentFile3(file, NULL, NULL, 0, NULL);
}

extern int ZEXPORT cpl_unzOpenCurrentFilePassword (unzFile file, const char*  password)
{
    return cpl_unzOpenCurrentFile3(file, NULL, NULL, 0, password);
}

extern int ZEXPORT cpl_unzOpenCurrentFile2 (unzFile file, int* method, int* level, int raw)
{
    return cpl_unzOpenCurrentFile3(file, method, level, raw, NULL);
}
